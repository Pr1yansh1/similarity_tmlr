...
def run_trial_for_num_reviewers(num_reviewers):
    ...
    for trial in range(1, 27):
        ...
        obj_scores_for_trial = [obj_score(scores, res) for res in [greedy_assign, rank_only_results, greedy_rt_results] + rank_greedy_results]
...
reviewers = [10, 15, 20, 25]

for idx, num in enumerate(reviewers):
    obj_scores = run_trial_for_num_reviewers(num)
    ax = axes[idx//2, idx%2]

    greedy_scores, rank_only, greedy_rt_results, *rank_greedy = list(zip(*obj_scores))
