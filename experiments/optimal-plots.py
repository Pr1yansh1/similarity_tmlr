...
_ALG_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "algorithms"))
...
print("\n Making plots ")
sim_scores = scores #np.loadtxt('similarity_result.txt')
d, lambd = 2, 1
obj_scores = []
assignments = []
...
for trial in range(20, 35):
    ...
    randomly_drawn_scores = scores # np.array(random.choices(scores, k = scores.shape[0]))
...
# Save to the location expected by rmab/main.tex
_OUT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "rmab", "result-stats"))
os.makedirs(_OUT_DIR, exist_ok=True)
plt.savefig("comparison-unfriendly-scores.png", dpi=200, bbox_inches="tight")
